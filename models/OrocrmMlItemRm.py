...
class OrocrmMlItemRm(Base):
    __tablename__ = 'orocrm_ml_item_rm'
    __table_args__ = (
        Index('orocrm_ml_list_ent_rm_unq', 'entity_id', 'marketing_list_id', unique=True),
    )

    id = Column(Integer, primary_key=True, server_default=text("nextval('orocrm_ml_item_rm_id_seq'::regclass)"))
