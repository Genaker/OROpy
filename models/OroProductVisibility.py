...
class OroProductVisibility(Base):
    __tablename__ = 'oro_product_visibility'
    __table_args__ = (
        Index('oro_prod_vis_uidx', 'product_id', 'scope_id', unique=True),
    )

    id = Column(Integer, primary_key=True, server_default=text("nextval('oro_product_visibility_id_seq'::regclass)"))
