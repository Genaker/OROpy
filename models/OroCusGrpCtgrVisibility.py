...
class OroCusGrpCtgrVisibility(Base):
    __tablename__ = 'oro_cus_grp_ctgr_visibility'
    __table_args__ = (
        Index('oro_cus_grp_ctgr_vis_uidx', 'category_id', 'scope_id', unique=True),
    )

    id = Column(Integer, primary_key=True, server_default=text("nextval('oro_cus_grp_ctgr_visibility_id_seq'::regclass)"))
