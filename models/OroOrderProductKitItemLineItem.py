...
class OroOrderProductKitItemLineItem(Base):
    __tablename__ = 'oro_order_product_kit_item_line_item'

    id = Column(Integer, primary_key=True, server_default=text("nextval('oro_order_product_kit_item_line_item_id_seq'::regclass)"))
    line_item_id = Column(ForeignKey('oro_order_line_item.id', ondelete='CASCADE'), nullable=False, index=True)
