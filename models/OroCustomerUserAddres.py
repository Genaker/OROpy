...
class OroCustomerUserAddres(Base):
    __tablename__ = 'oro_customer_user_address'

    id = Column(Integer, primary_key=True, server_default=text("nextval('oro_customer_user_address_id_seq'::regclass)"))
    ...
    frontend_owner_id = Column(ForeignKey('oro_customer_user.id', ondelete='CASCADE'), index=True)
